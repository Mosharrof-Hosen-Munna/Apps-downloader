...
const Navbar = () => {
  ...
  return (
    <nav className="bg-white shadow-sm border-b border-gray-100 font-sans sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
        <div className="flex justify-between items-center py-3">
          ...
          <NavLink to="/" className="shrink-0 flex items-center gap-3">
            <img className='w-10' src={logo} alt="logo image" /><span className="text-[22px] font-black tracking-tight text-[#7C3AED]">AppStack</span>
          </NavLink>
          ...
          <div className="flex items-center">
            ...
            >
              {isOpen ? (
                ...
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M4 12h16M4 6h16M4 18h16"/></svg>
              )}
            </button>
          </div>
        </div>
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-300 ${isOpen ? 'max-h-64 border-t' : 'max-h-0'}`}>
        <div className="px-6 py-4 space-y-4 bg-white shadow-inner">
          {navLinks.map((link) => (
            <NavLink
              ...
              className={({ isActive }) => `block text-lg font-bold ${isActive ? 'text-[#7C3AED]' : 'text-gray-600'}`}
            >
              {link.name}
            </NavLink>
          ))}
          <a href="#" className="flex w-full items-center justify-center bg-[#7C3AED] text-white py-3 rounded-lg font-bold">
            Contribute
          </a>
        </div>
      </div>
